...
ggplot(wildfire_clean_50, aes(x = Prec_pre_15, y = fire_size, color = Vegetation)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE, color = "red") +
...
fire_main_fit <- lm(log(fire_size) ~ remoteness + Prec_pre_15 + Temp_pre_15 + Vegetation + stat_cause_descr + Wind_cont, data = wildfire_clean_50)

fire_int_fit <- lm(log(fire_size) ~ remoteness + Temp_pre_15 + Prec_pre_15 * Vegetation + stat_cause_descr + Wind_cont, data = wildfire_clean_50)

---
title: "Analyzing Factors Associated with Wildfire Burned Area in the United States"
...
p5 <- ggplot(wildfire_clean, aes(x = Precipitation)) +
  geom_bar(fill = "cadetblue4", color = "white", alpha = 0.8) +
  labs(title = "Precipitation", subtitle = "15 days prior to discovery", x = NULL, y = "Count") +
  base_theme
...
wildfire_clean_long <- wildfire_clean |>
  pivot_longer(
    cols = c(Vegetation_group, cause_category, season, region),
    names_to = "Group_Type",
    values_to = "Group"
  ) |>
  mutate(

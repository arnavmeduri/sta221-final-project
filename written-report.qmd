...
```{r}
ggplot(wildfire_clean, aes(y = fct_infreq(cause_category))) +
...
```{r}
ggplot(wildfire_clean, aes(x = as.factor(Vegetation), y = fire_size)) +
...
summary(wildfire_clean$remoteness)
...
ggplot(exp_fire_fit_aug, aes(x = disc_clean_date, y = .resid)) +
  ...
  labs(
    title = "Temporal Distribution Residuals Over Time",
    x = "Discovery Date",
    y = "Residual"
  ) +
  theme_minimal()

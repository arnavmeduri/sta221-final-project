---
title: "Investigating Geographic and Environmental Correlates of Wildfire Size in the United States"
...
wildfire_clean <- wildfire |>
  ...
  mutate(
    ...
    cause_category = case_when(
      ...
      stat_cause_descr %in% c("Campfire", "Children", 
                              "Fireworks", "Smoking") ~ "Recreational",
      stat_cause_descr == "Debris Burning" ~ "Debris Burning",
      stat_cause_descr == "Arson" ~ "Arson",
      stat_cause_descr %in% c("Equipment Use", "Powerline", 
                              "Railroad", "Structure") ~ "Infrastructure",
      stat_cause_descr %in% c("Miscellaneous", "Missing/Undefined") ~ "Other/Unknown",
      TRUE ~ "Other"
    ) |> factor()
  ) |>
  drop_na(fire_mag, stat_cause_descr, Vegetation, remoteness, fire_size, putout_time,
...
```{r}
ggplot(wildfire_clean, aes(y = fct_infreq(stat_cause_descr))) +
...
**Model - Explainability (Forward Selection using Adj. R\^2)**

```{r}

fire_size_fit <- lm(fire_size ~ region + cause_category + remoteness + putout_time_num +
              Wind_pre_15 + Hum_pre_15 + Temp_pre_15 + Prec_pre_15, data = wildfire_clean)

tidy(fire_size_fit)

glance(fire_size_fit)
```

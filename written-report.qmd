...
main_aug <- augment(fire_main_fit, data = wildfire_clean_50) |>
  mutate(truth = log(fire_size), estimate = .fitted)

int_aug <- augment(fire_int_fit, data = wildfire_clean_50) |>
  mutate(truth = log(fire_size), estimate = .fitted)
...
main_bin_aug <- augment(fire_main_bin, data = wildfire_clean_50) %>%
  mutate(truth = log(fire_size), estimate = .fitted)

int_bin_aug <- augment(fire_int_bin, data = wildfire_clean_50) %>%
  mutate(truth = log(fire_size), estimate = .fitted)
